...
class FhmEventListener
{
    ...
    public function onKernelRequest(GetResponseEvent $event)
    {
        ...
        // Twig global
        $parameters = $container->getParameter("fhm_fhm");
        $container->get($parameters["grouping"])->loadTwigGlobal();
        $container->get('project_twig_global')->load();
        // Locale
        ...
        $parameters  = $container->getParameter("project");
        ...
        $authorized  = $container->get('security.context')->isGranted('ROLE_ADMIN') ? true : $authorized;
        $authorized  = in_array($container->get('kernel')->getEnvironment(), array('test', 'dev')) ? true : $authorized;
        if($construct && !$authorized)
        {
            $event->setResponse(new Response($container->get('templating')->render('::ProjectDefault/Template/construct.html.twig', array('date' => $date, 'site' => $this->container->get($this->container->getParameter("fhm_fhm")["grouping"])->getSite())), 503));
            $event->stopPropagation();
        }
        elseif($maintenance && !$authorized)
        {
            $event->setResponse(new Response($container->get('templating')->render('::ProjectDefault/Template/maintenance.html.twig', array('date' => $date, 'site' => $this->container->get($this->container->getParameter("fhm_fhm")["grouping"])->getSite())), 503));
            $event->stopPropagation();
        }
