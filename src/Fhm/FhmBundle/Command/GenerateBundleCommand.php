...
use Fhm\FhmBundle\Generator\BundleGenerator;
...
<info>php app/console fhm:generate:bundle --namespace=Acme/BlogBundle --dir=src [--bundle-name=...] --no-interaction</info>
...
EOT
    ...
    {
        $questionHelper = $this->getQuestionHelper();
        $questionHelper->writeSection($output, 'Welcome to the FHM bundle generator');
        // namespace
        $namespace = null;
        try
        {
        ...
        $output->writeln(array(
            ...
            '',
            sprintf("You are going to generate a \"<info>%s\\%s</info>\" bundle\nin \"<info>%s</info>\" using the \"<info>%s</info>\" format.", $namespace, $bundle, $dir, 'annotation'),
            '',
        ));
    }
