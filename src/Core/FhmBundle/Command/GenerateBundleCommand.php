<?php
namespace Core\FhmBundle\Command;
 ...
 * Time: 17:52
 */
use Core\FhmBundle\Generator\BundleGenerator;
...
class GenerateBundleCommand extends GeneratorCommand
{
    ...
    protected function configure()
    {
        $this
            ...
            ->setHelp( <<<EOT
                        The <info>fhm:generate:bundle</info> command helps you generates new Fhm bundles.
                        ...
                        <info>php app/console fhm:generate:bundle --namespace=Acme/BlogBundle</info>
                        
                        Note that you can use <comment>/</comment> instead of <comment>\\ </comment>
                        ...
                        <info>
                            php app/console fhm:generate:bundle --namespace=Acme/BlogBundle 
                                                                --dir=src [--bundle-name=...] --no-interaction</info>
                        
                        Note that the bundle namespace must end with "Bundle".
EOT
            )
            ->setName('fhm:generate:bundle');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($input->isInteractive()) {
            $conf = new ConfirmationQuestion($helper->getQuestion('Do you confirm generation', 'yes', '?'), true);
            if (!$helper->ask($input, $output, $conf)) {
...
//        $dir    = Validators::validateTargetDir($input->getOption('dir'), $bundle, $namespace);
        ...
        $runner($this->updateKernel($output, $this->getContainer()->get('kernel'), $namespace, $bundle)
        );
        // routing
        $runner($this->updateRouting($output, $bundle, 'annotation'));
    ...
    {
        $helper = $this->getQuestionHelper();
        $helper->writeSection($output, 'Welcome to the FHM bundle generator');
        // namespace
        $namespace = null;
        try {
            // validate the namespace option (if any) but don't require the vendor namespace
            $namespace = $input->getOption('namespace') ?
                                Validators::validateBundleNamespace($input->getOption('namespace'), false) : null;
        } catch (\Exception $error) {
            $output->writeln($helper->getHelperSet()->get('formatter')->formatBlock($error->getMessage(), 'error'));
        }
        if (null === $namespace) {
            $output->writeln(array(
                ...
                '',
                'Use <comment>/</comment> instead of <comment>\\ </comment> 
                  for the namespace delimiter to avoid any problem.',
                '',
            ));
            ...
            while (!$acceptedNamespace) {
                ...
                if (false === strpos($namespace, '\\')) {
                    ...
                    $msg   = array();
                    $msg[] = '';
                    $msg[] = sprintf('The namespace sometimes contain a vendor namespace (e.g. 
                    <info>VendorName/BlogBundle</info> instead of simply <info>%s</info>).', $namespace, $namespace);
                    $msg[] = 'If you\'ve *did* type a vendor namespace, try using a forward slash 
                    <info>/</info> (<info>Acme/BlogBundle</info>)?';
                    $msg[] = '';
                    $output->writeln($msg);
                    $question          = new ConfirmationQuestion($helper->getQuestion(
                        sprintf(
                            'Keep <comment>%s</comment> as the bundle namespace (choose no to try again)?',
                            $namespace
                        ),
                        'yes'
                    ), true);
                    $acceptedNamespace = $helper->ask($input, $output, $question);
                }
            }
        ...
        if (null === $bundle) {
            ...
            $question = new Question($helper->getQuestion('Bundle name', $bundle), $bundle);
...
//                null;
        ...
        if (null === $dir) {
            ...
            $question = new Question($helper->getQuestion('Target directory', $dir), $dir);
            $question->setValidator(
    ...
    protected function updateKernel(OutputInterface $output, KernelInterface $kernel, $namespace, $bundle)
    {
        $auto = true;
    ...
    protected function updateRouting(OutputInterface $output, $bundle, $format)
    {
        $auto = true;
        $kernelDir = $this->getContainer()->getParameter('kernel.root_dir');
        $output->write('Importing the bundle routing resource: ');
        $routingFile = substr($bundle, 0, 7) == 'Project' ?
            'routing_project' :
            (
                substr($bundle, 0, 3) == 'Shop' ?
                    'routing_shop' :
                    (
                        substr($bundle, 0, 3) == 'Fhm' ?
                            'routing_fhm' :
                            'routing_other'
                    )
            );
        $routing = new RoutingManipulator($kernelDir . '/config/' . $routingFile . '.yml');
        try {
