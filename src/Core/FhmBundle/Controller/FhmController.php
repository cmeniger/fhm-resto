...
class FhmController extends Controller
{
    ...
    {
        $this->initLanguage();
        $fhmExtension      = new \Fhm\FhmBundle\Twig\FhmExtension($this->get('service_container'));
    ...
    public function initLanguage()
    {
        ...
        if(!$this->language_disable && $this->getParameter(array('languages', 'codes'), 'fhm_fhm'))
        {
            ...
            $session        = $this->get('session')->get('_locale');
            ...
            $used           = $this->get('security.authorization_checker')->isGranted('ROLE_ADMIN') ? $this->get('session')->get('_localeAdmin') : $used;
            $this->language = $used;
        }
    ...
    protected function getSort($field = "", $order = "", $post = "", $pagination = 'pagination', $path = "")
    {
        ...
        $sort               = new \stdClass();
        $sort->path         = $path ? $path : $this->generateUrl($this->get('request')->get('_route'));
    ...
    protected function getPagination($page = 1, $countPage = 1, $countAll = 1, $tag = 'pagination', $post = array(), $path = "")
    {
        ...
        $pagination->path         = $path ? $path : $this->generateUrl($this->get('request')->get('_route'));
    ...
    {
        $parameters = $this->get('service_container')->getParameter($parent);
        $value      = $parameters;
